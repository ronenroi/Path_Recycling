...
class SceneRR(object):
    ...
    def build_paths_list(self, Np, to_print=False):
        ...
        # sorted_inds = np.argsort(dpath_sizes.copy_to_host())
        ...
        scatter_inds = np.concatenate([np.array([0]), scatter_sizes])
        scatter_inds = np.cumsum(scatter_inds)
        dsorted_scatter_inds = cuda.to_device(scatter_inds)

        self.sort_scatter_points[self.blockspergrid, self.threadsperblock]\
            (dscatter_points, dstarting_points, sorted_inds, dsorted_scatter_inds, dscatter_inds, dsorted_scatter_points,
             dsorted_starting_points)
        # scatter_points = dscatter_points.copy_to_host()
        # counter = 0
        # for n in range(scatter_sizes.shape[0]):
        #     index = sorted_inds[n]
        #     sorted_scatter_points[:,counter:counter+scatter_sizes[index]] = scatter_points[:,scatter_inds[index]:scatter_inds[index]+scatter_sizes[index]]
        ...
        return dsorted_starting_points, dsorted_scatter_points, dsorted_scatter_inds
...
class SceneRR_batch(object):
                ...
                for seg in range(N_seg):
                    ...
                    if seg > 0:
                        if cloud_mask[current_voxel[0], current_voxel[1], current_voxel[2]]:
                            ...
                            seg_contrib = omega_fp_c / \
                                          (beta_c * omega_fp_c + beta_air * w0_air * rayleigh_pdf(cos_theta_scatter)) # scatter fix
                            cuda.atomic.add(total_grad, (grad_batch, current_voxel[0], current_voxel[1], current_voxel[2]), seg_contrib * grad_temp)
                    ###########################################################
                    ############## voxel_fixed traversal_algorithm_save #############
                    path_size = estimate_voxels_size(dest_voxel, current_voxel)
                    for pi in range(path_size):
                        ...
                        if cloud_mask[current_voxel[0], current_voxel[1], current_voxel[2]]:
                            grad = -length * grad_temp
                            cuda.atomic.add(total_grad, (grad_batch, current_voxel[0],current_voxel[1],current_voxel[2]), grad)
                        current_voxel[border_ind] += sign(direction[border_ind])
                        step_in_direction(current_point, direction, length)
                    ...
                    for k in range(N_cams):
                        ...
                        if pixel[0] != 255:
                            ...
                            if cloud_mask[camera_voxel[0], camera_voxel[1], camera_voxel[2]]:
                                ...
                                seg_contrib = omega_fp_c / \
                                              (beta_c * omega_fp_c + beta_air * w0_air * rayleigh_pdf(cos_theta))
                                cuda.atomic.add(total_grad, (grad_batch, camera_voxel[0],camera_voxel[1],camera_voxel[2]), seg_contrib * grad_temp)
                            ###########################################################################
                            ######################## local estimation save ############################
                            for pi in range(path_size):
                                ##### border traversal #####
                                ...
                                if cloud_mask[camera_voxel[0], camera_voxel[1], camera_voxel[2]]:
                                    grad = -length * grad_temp
                                    cuda.atomic.add(total_grad,(grad_batch, camera_voxel[0], camera_voxel[1], camera_voxel[2]), grad)
                                # next point and voxel
                                camera_voxel[border_ind] += sign(cam_direction[border_ind])
                                step_in_direction(camera_point, cam_direction, length)
                            # Last Step
    ...
    def build_paths_list(self, Np, to_print=False, to_sort=True):
        # inputs
        del(self.dpath_contrib)
        del(self.dgrad_contrib)
        self.init_cuda_param(Np, self.Np!=Np)
...
class LargeDomainSceneRR(object):
    def __init__(self, volume: Volume, cameras, sun_angles, g_cloud, rr_depth, rr_stop_prob):
        self.rr_depth = rr_depth
        self.rr_stop_prob = rr_stop_prob
        rr_factor = 1.0 / (1.0 - self.rr_stop_prob)
        ...
        def calc_scatter_sizes(Np, beta_cloud, beta_air, g_cloud, w0_cloud, w0_air, bbox, bbox_size, voxel_size, sun_direction,
            ...
            if tid < Np:
                grid_shape = beta_cloud.shape
                # local memory
                current_voxel = cuda.local.array(3, dtype=np.uint16)
                next_voxel = cuda.local.array(3, dtype=np.uint16)
                starting_point = cuda.local.array(3, dtype=float_precis)
                current_point = cuda.local.array(3, dtype=float_precis)
                direction = cuda.local.array(3, dtype=float_precis)
                new_direction = cuda.local.array(3, dtype=float_precis)
        ...
        def render_cuda(beta_cloud, beta_zero, beta_air, g_cloud, w0_cloud, w0_air, bbox, bbox_size, voxel_size, N_cams,
            ...
            if tid < scatter_inds.shape[0] - 1:
                ...
                current_voxel = cuda.local.array(3, dtype=np.uint16)
                # next_voxel = cuda.local.array(3, dtype=np.uint16)
                dest_voxel = cuda.local.array(3, dtype=np.uint16)
                camera_voxel = cuda.local.array(3, dtype=np.uint16)
                current_point = cuda.local.array(3, dtype=float_precis)
                camera_point = cuda.local.array(3, dtype=float_precis)
                next_point = cuda.local.array(3, dtype=float_precis)
                direction = cuda.local.array(3, dtype=float_precis)
...
class PytorchSceneRR(SceneRR_batch):
    def __init__(self, volume: Volume, cameras, sun_angles, g_cloud, rr_depth, rr_stop_prob, N_batches, device):
        super(PytorchSceneRR, self).__init__(volume,cameras,sun_angles,g_cloud,rr_depth,rr_stop_prob, N_batches)
        self.device = device
