...
class SceneSeed_batch(object):
    ...
    def build_paths_list(self, Np, init=False, to_sort=True, to_print=False):
        ...
        cuda.synchronize()
        if to_print:
            print("calc scatter sizes took:",time.time()-start)
        ...
        if to_sort:
            ...
            # print("lexsort took:", time.time()-start)
    ...
    def render(self, I_gt=None, to_print=False):
        ...
        if I_gt is None:
            if to_print:
                print("render_cuda took (once):", time.time() - start)
            # del dpath_contrib
            return I_total
        ...
        if to_print:
            print("render_cuda took (twice):",time.time() - start)
...
class PytorchSceneSeed(SceneSeed_batch):
    def __init__(self, volume: Volume, cameras, sun_angles, g_cloud, rr_depth, rr_stop_prob, device, N_batches=1):
        super(PytorchSceneSeed, self).__init__(volume,cameras,sun_angles,g_cloud,rr_depth,rr_stop_prob, N_batches)
        self.device = device

    def render(self, I_gt=None, to_print=False, to_torch=False):
        assert self.Np % self.N_batches == 0, "N_batches must divide Np"
        # east declerations
        N_cams = len(self.cameras)
        pixels_shape = self.cameras[0].pixels
        ...
        self.render_cuda[blockspergrid, threadsperblock] \
            (self.dbeta_cloud, self.dbeta_zero, beta_air, g_cloud, w0_cloud, w0_air, self.dsun_direction, self.dbbox,
             self.dbbox_size,
             self.dvoxel_size, N_cams, self.dts, self.dPs, self.dpixels_shape, self.dis_in_medium, self.drng_states,
             self.dscatter_inds, self.dI_total,
             self.dgrad_contrib)
        cuda.synchronize()
