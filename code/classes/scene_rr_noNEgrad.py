...
class SceneRR_noNE(object):
    def __init__(self, volume: Volume, cameras, sun_angles, g_cloud, rr_depth, rr_stop_prob):
        ...
        @cuda.jit()
        def calc_gradient_contribution_fast(scatter_points, path_contrib, I_diff, Ps, pixel_shape, scatter_inds, grad_contrib):
            tid = cuda.grid(1)
            if tid < scatter_inds.shape[0] - 1:
                pixel = cuda.local.array(2, dtype=np.uint8)
                scatter_ind = scatter_inds[tid]
                ...
                for seg in range(N_seg):
                    ...
                    for k in range(path_contrib.shape[0]):
                        ...
                        if pixel[0] != 255:
                            grad_sum += path_contrib[k, seg_ind] * I_diff[k,pixel[0],pixel[1]]
        ...
        def render_differentiable_cuda(beta_cloud, beta_air, g_cloud, w0_cloud, w0_air, bbox, bbox_size, voxel_size, starting_points, scatter_points, scatter_inds,
                                       grad_contrib, cloud_mask, total_grad):
            tid = cuda.grid(1)
            if tid < scatter_inds.shape[0] - 1:
                scatter_ind = scatter_inds[tid]
                N_seg = scatter_inds[tid + 1] - scatter_ind
        ...
        self.calc_gradient_contribution = calc_gradient_contribution_fast
    ...
    def render(self, cuda_paths, I_gt=None, to_print=False):
        ...
        self.render_differentiable_cuda[blockspergrid, threadsperblock]\
            (self.dbeta_cloud, beta_air, g_cloud, w0_cloud, w0_air, self.dbbox, self.dbbox_size, self.dvoxel_size,
                                   *cuda_paths, self.dgrad_contrib, self.dcloud_mask, self.dtotal_grad)
...
class SceneRR_noNE_batched(object):
    def __init__(self, volume: Volume, cameras, sun_angles, g_cloud, rr_depth, rr_stop_prob, N_batches=1):
        ...
        @cuda.jit()
        def render_differentiable_cuda(beta_cloud, beta_air, g_cloud, w0_cloud, w0_air, bbox, bbox_size, voxel_size, starting_points, scatter_points, scatter_inds,
                                       grad_contrib, cloud_mask, total_grad, N_batches):
            tid = cuda.grid(1)
            if tid < scatter_inds.shape[0] - 1:
                scatter_ind = scatter_inds[tid]
                N_seg = scatter_inds[tid + 1] - scatter_ind
    ...
    def render(self, cuda_paths, I_gt=None, to_print=False):
        ...
        self.render_differentiable_cuda[blockspergrid, threadsperblock]\
            (self.dbeta_cloud, beta_air, g_cloud, w0_cloud, w0_air, self.dbbox, self.dbbox_size, self.dvoxel_size,
                                   *cuda_paths, self.dgrad_contrib, self.dcloud_mask, self.dtotal_grad, self.N_batches)
        ...
        MC_frac = (self.Np * N_cams) / self.N_batches
        total_grad /= MC_frac
