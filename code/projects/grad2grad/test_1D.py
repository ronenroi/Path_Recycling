...
if __name__ == '__main__':
    ...
    for ind, file in enumerate(files):
        ...
        for use_nn, Np_factor in zip([True, False, False],Np_factors):
            ...
            for it in range(iterations):
                ...
                if use_nn and it>=0:
                    ...
                    nn_grad_list.append(torch.squeeze(total_grad).to("cpu").detach().numpy().reshape(10,32))
        ...
        plt.savefig(join("/home/roironen/Path_Recycling/code/test_results/1D_test_results1", "eps_cloud{}_iterations".format(ind)), bbox_inches='tight')
        plt.close()
        ...
        plt.yscale('log')
        plt.savefig(
            join("/home/roironen/Path_Recycling/code/test_results/1D_test_results1", "loss_cloud{}_iterations".format(ind)),
            bbox_inches='tight')
        plt.close()
    ...
    print(f'Noisy best relative errors {table_noisy}, mean = {np.mean(table_noisy)}, mean = {np.std(table_noisy)}')
    print(f'Clean best relative errors {table_clean}, mean = {np.mean(table_clean)}, mean = {np.std(table_clean)}')
    print(f'NN best relative errors {table_nn}, mean = {np.mean(table_nn)}, mean = {np.std(table_nn)}')