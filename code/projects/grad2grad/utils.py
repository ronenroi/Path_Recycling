...
def plot_per_epoch(ckpt_dir, title, measurements, legends, yaxis='linear'):
    """Plots stats (train/valid loss, avg PSNR, etc.)."""

    fig = plt.figure()
    ax = fig.add_subplot(111)
    for measurement,legend in zip(measurements, legends):
        ax.plot(range(1, len(measurement) + 1), measurement, label=legend)
