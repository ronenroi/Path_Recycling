...
class Cloud_Flows(nn.Module):
    ...
    def __init__(self, args):
        super(Cloud_Flows, self).__init__()
        ...
        self.alpha_mean = torch.nn.Parameter(torch.zeros(1),requires_grad=False)
        self.alpha_std = torch.nn.Parameter(torch.ones(1),requires_grad=False)

        self.interpolation_alpha = torch.empty([2,1]).normal_()

        self.sample_size = args.K_samples
        self.sample_alpha = torch.empty([self.sample_size,1]).normal_()

        self.output_linear = nn.Linear(self.W, self.h_alpha_size)
        
        self.flows_alpha = TriangularSylvesterNeRF(args,'alpha')
        # self.sm = nn.Softplus()
    
    def sample(self, x):
        h_alpha = self.encode(x)
        ######################## entropy loss ########################
        BN = h_alpha.shape[0]
        ## reparameterizate 
        # density
    ...
    def interpolation(self, x, is_val=False):
        ...
        eps_alpha = self.interpolation_alpha[None,...].expand_as(alpha_mean_k)
    ...
    def rand(self, shape):
        alpha_mean_k = self.alpha_mean[None, None, :].expand([shape, self.K_samples, 1])
        eps_alpha = torch.empty([self.K_samples, 1]).normal_()
        eps_alpha = eps_alpha[None, ...].expand_as(alpha_mean_k).to(self.device)
        self.eps_alpha = eps_alpha
    def forward(self, x, is_rand=True, is_val=False, is_test=False):
        ...
        if is_test:
            ...
            return z_k_alpha, torch.zeros_like(z_k_alpha)
        ...
        ## sum up loss_entropy
        loss_entropy = base_log_norm_alpha.mean() - sum_log_det_j_alpha.mean()
        return z_k_alpha, loss_entropy#.expand_as(z_k_alpha)
