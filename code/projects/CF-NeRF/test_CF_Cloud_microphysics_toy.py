...
class OptimizationScript(ExtinctionOptimizationScript):
    ...
    def neuralnetwork_args(self, parser):
        ...
        # parser.add_argument("--ft_path", type=str, default='/home/roironen/CF-NeRF/pyshdom-NN/experiments/WIZ_10m_1sec_micro_256_0000004400_perspective/monochromatic/logs/lwc_recover-04-Jan-2023-15:20:35/23200.tar',
        parser.add_argument("--ft_path", type=str, default='/home/roironen/CF-NeRF/pyshdom-NN/experiments/WIZ_10m_1sec_micro_256_0000004400_perspective/monochromatic/logs/lwc_recover-05-Jan-2023-13:33:33/11200.tar',
    ...
    def main(self):
        ...
        self.test(local_optimizer, ground_truth, rte_solver, measurements)





    def test(self, optimizer, ground_truth, rte_solver, measurements):
        ...
        model = Cloud_Flows(self.args).to(device).eval()
        ...
        def forward_model(x):
            ...
            x_down = up(x[..., :y_down, :]).flatten()
            x_up = down(x[..., y_down:, :]).flatten()
        ...
        for iter in range(200):
            ...
            if optimizer.writer.epsilon<0.6:
                cloud_list.append(xx.detach().cpu().numpy())
            else:
                print('Dropped cloud too large epsilon')

            print('Sample {}: Image Loss: {} Entropy Loss: {} relative_error: {}'.format(iter, loss,
                                                                                        loss_entropy.item(),
                                                                                        relative_error_list[
                                                                                            -1]))
        ...
        path = os.path.join(optimizer.writer.dir,
                            'results.np')
        np.save(path,{'est_clouds':cloud_list,'gt_clouds':ground_truth.lwc.data})

        e = [(c-ground_truth.lwc.data)[ground_truth.lwc.data>0] for c in cloud_list]
        e = np.array(e)
        voxel_std = np.std(e,0)
        voxel_mean = np.mean(e, 0)
        plt.hist(voxel_std, 20)
        plt.show()
        plt.hist(voxel_mean, 20)
        plt.show()

        c_up = []
        c_down = []
        for c in cloud_list:
            e = c - ground_truth.lwc.data
            c_up.append( e[...,:int(c.shape[-2]/2),:])
            c_down.append(e[...,int(c.shape[-2]/2):,:])


        e_up = [c[ground_truth.lwc.data[:,:int(c.shape[-2]),:]>0] for c in c_up]
        e_up = np.array(e_up)
        e_down = [c[ground_truth.lwc.data[:,7:,:]>0] for c in c_down]
        e_down = np.array(e_down)
        voxel_std_down = np.std(e_down,0)
        voxel_std_up = np.std(e_up,0)
        voxel_mean_down = np.mean(e_down,0)
        voxel_mean_up = np.mean(e_up, 0)
        plt.hist(voxel_std_down, 20)
        plt.hist(voxel_std_up, 20,alpha=0.5)
        plt.show()
        plt.hist(voxel_mean_down, 10)
        plt.hist(voxel_mean_up, 10,alpha=0.5)
        plt.legend(['no averaging','averaging'])
        plt.show()
        ...
        for c in cloud_list:
            c_up= c[...,:int(c.shape[-2]/2),:]
            c_down = c[...,int(c.shape[-2]/2):,:]
            gt_up = ground_truth.lwc.data[:, :int(c.shape[-2]/2), :]
            gt_down = ground_truth.lwc.data[:, int(c.shape[-2]/2):, :]
            ax2.scatter(gt_up,c_up)
        ax2.plot([0,0.4], [0,0.4], c='r', ls='--')
        for c in cloud_list:
            c_up= c[...,:int(c.shape[-2]/2),:]
            c_down = c[...,int(c.shape[-2]/2):,:]
            gt_up = ground_truth.lwc.data[:, :int(c.shape[-2]/2), :]
            gt_down = ground_truth.lwc.data[:, int(c.shape[-2]/2):, :]
            ax3.scatter(gt_down,c_down)
        ax3.plot([0,0.4], [0,0.4], c='r', ls='--')
