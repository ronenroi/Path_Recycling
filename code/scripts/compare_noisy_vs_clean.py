...
for file in files:
    ...
    for state in [1, 10]:
        ...
        for iter in range(iterations):
            ...
            start = time()
            del(cuda_paths)
            cuda_paths = scene_rr.build_paths_list(Np*state)
            end = time()
        ...
        if state==1:
            noisy_err_list.append(np.array(err))
            noisy_time_list.append(time_list)
        else:
            clean_err_list.append(np.array(err))
            clean_time_list.append(time_list)
    print("{} = {} | {} = {}".format("noisy", noisy_err_list[-1][-1], "clean", clean_err_list[-1][-1]))
    plt.plot(noisy_err_list[-1])
    plt.plot(clean_err_list[-1])
    plt.legend(['noisy', 'clean'])
    plt.xlabel("Iterations")
    plt.ylabel("Relative error")
    plt.show()

    plt.plot(np.cumsum(noisy_time_list[-1]), noisy_err_list[-1])
    plt.plot(np.cumsum(clean_time_list[-1]), clean_err_list[-1])
    plt.legend(['noisy', 'clean'])
    plt.xlabel("Time [sec]")
    plt.ylabel("Relative error")
    plt.show()
